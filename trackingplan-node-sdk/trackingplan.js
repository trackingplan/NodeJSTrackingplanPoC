...
// Enabled interception modes. Available are 'img', 'xhr', 'beacon', 'form', 'frame', 'ws', 'fetch'
var _intercept = ['msw'];
...
function sendTPEvent(payload) {
    var request = { "method": "POST", "endpoint": "TRACKINGPLAN", "payload": JSON.stringify(payload) };
...
function contentFilterRequest(request, contentFilters) {
    ...
    for (var i = 0; i < contentFilters.length; i++) {
        if ('payload' in request && typeof request['payload'] === 'string') {
            if (request.payload.includes(contentFilters[i])) {
                return true;
            }
        }
        if ('endpoint' in request && typeof request['endpoint'] === 'string') {
            if (request.endpoint.includes(contentFilters[i])) {
                return true;
            }
        }
    }
    ...
    xmlhttp.onreadystatechange = function () {
        if (this.readyState == 4) {
            try {
                ...
                //sendTPEvent({ "event_name": "new_dau" });
                processPreQueue();
            } catch (error) { };
        }
    };
